...
export class Store {
    ...
    dispatch<T>(actionName: string, nameSpace: string, payload: T) {
        if (
            this.reducerStore.hasOwnProperty(nameSpace) &&
            this.reducerStore[nameSpace].hasOwnProperty(actionName)
        ) {
    ...
    private makeProxy() {
        ...
        this.state = new Proxy(this.state, {
            set(target, property, value) {
                ...
                self.eventBus.emit(`${Store.EVENTS.CHANGE}${property as String}`, value);
                return true;
            },
        });
    }
